import { IState, IRow, ICell } from "../common/interfaces";
...
export const addObstacles = (map: IState['map'], density = 10) => map.map(row => row.filter(() => Math.floor(Math.random() * 100) + 1 < density).map(cell => {
    const wall = {
        position: cell.position,
        location: '',
        blocker: true,
